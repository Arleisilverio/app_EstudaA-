...
import { Send, Sparkles, BookOpen, ListChecks, GraduationCap, Loader2, Info } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-6 flex-1 w-full max-w-3xl mx-auto">
      <div className="space-y-2">
        <h2 className="text-2xl font-bold text-study-dark flex items-center gap-2">
          Pergunte ao Estuda AÍ
          <Sparkles className="text-study-primary" size={20} />
        </h2>
        <p className="text-study-medium">Faça perguntas complexas com base nos seus arquivos carregados.</p>
      </div>

      <form onSubmit={handleAsk} className="relative group">
        <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
          <Sparkles className="text-study-medium group-focus-within:text-study-primary transition-colors" size={20} />
        </div>
        <Input
          placeholder="Digite sua pergunta sobre o conteúdo..."
          className="pl-12 pr-24 py-8 rounded-2xl border-none shadow-study bg-white text-lg focus-visible:ring-2 focus-visible:ring-study-primary/20 transition-all"
          value={query}
      ...
      <AnimatePresence mode="wait">
        ...
        {response && !isLoading && (
          <motion.div
            ...
            className="space-y-6"
          >
            <Card className="border-none shadow-study bg-white overflow-hidden rounded-2xl">
              <CardContent className="p-8">
                <div className="prose prose-study max-w-none">
                  {response.text.split('\n').map((line, i) => (
                    <p key={i} className="text-study-dark leading-relaxed mb-4">
                      {line}
                    </p>
                  ))}
                </div>

                <div className="mt-8 pt-6 border-t border-study-light/30">
                  <div className="flex items-center gap-2 text-study-medium mb-3">
                    <Info size={16} />
                    <span className="text-xs font-bold uppercase tracking-wider">Fontes utilizadas:</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {response.sources.map((source, i) => (
                      <span key={i} className="px-3 py-1 rounded-full bg-study-light/30 text-study-primary text-xs font-medium border border-study-light/50">
                        {source}
                      </span>
                    ))}
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <Button variant="outline" className="h-auto py-4 rounded-xl border-study-light hover:bg-study-light/20 flex flex-col gap-2 group transition-all">
                <div className="bg-study-light p-2 rounded-lg group-hover:bg-white transition-colors">
                  <BookOpen size={20} className="text-study-primary" />
                </div>
                <div className="text-center">
                  <p className="font-bold text-study-dark text-sm">Gerar Resumo</p>
                  <p className="text-[10px] text-study-medium">Síntese do tópico</p>
                </div>
              </Button>
              <Button variant="outline" className="h-auto py-4 rounded-xl border-study-light hover:bg-study-light/20 flex flex-col gap-2 group transition-all">
                <div className="bg-study-light p-2 rounded-lg group-hover:bg-white transition-colors">
                ...
                <div className="text-center">
                  <p className="font-bold text-study-dark text-sm">Criar Questões</p>
              ...
              <Button variant="outline" className="h-auto py-4 rounded-xl border-study-light hover:bg-study-light/20 flex flex-col gap-2 group transition-all">
                ...
                <div className="text-center">
                  <p className="font-bold text-study-dark text-sm">Simulado</p>
        ...
        {!response && !isLoading && (
          ...
          >
            <div className="bg-study-light/50 p-6 rounded-full mb-4">
              <BookOpen size={48} className="text-study-medium" />
            </div>
            <p className="text-lg font-medium text-study-dark">Pronto para começar!</p>
