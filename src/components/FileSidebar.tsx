...
const FileSidebar = () => {
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success("Documento adicionado à base de estudos!");
      fetchDocuments();
    } catch (err: any) {
  ...
  const removeDoc = async (id: string) => {
    if (!confirm("Excluir documento da base de estudos?")) return;
    try {
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      case 'ready': 
        return { label: 'Disponível', color: 'text-green-500', icon: CheckCircle2 };
      case 'processing': 
        return { label: 'Lendo...', color: 'text-study-primary', icon: Loader2 };
      case 'error': 
          ...
          <ScrollArea className="h-[400px] px-4 py-4">
            <div className="flex flex-col gap-3">
              {loading ? (
                ...
                <div className="text-center py-10 opacity-30 text-[10px] font-black uppercase">Nenhum arquivo nesta matéria</div>
              ) : (
                documents.map((doc) => {
